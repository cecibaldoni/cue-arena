...
setwd("~/home/cbaldoni/data/cue_learning/pilot")
doors <- read.csv("/home/cbaldoni/data/cue_learning/pilot/trial_door.csv") %>%
  mutate(Trial = paste0("T",trial_n))

coords <- read.csv("~/data/cue_learning/pilot/food_door_coordinates.csv", sep = ",",header = TRUE) %>%
...
tracking <- read.csv("/home/ceci/data/cue_learning/data.csv") #this is the one with all tracking data together
...
other_door_visits_ls <- lapply(trial_ls, function(x){
  ...
  if (sum(track_sf_2$food_journey == "exploration") > 0) { #some problems here, exploration is sometimes negative
    ...
    df = data.frame(x[1,1],x[1,3],x[1,2] ,  dist_doorfood, walk_to, walk_back, walk_expl, straight_index_to, straight_index_back, straight_exploration)
    colnames(df)[1] = "ID"
    ...
    colnames(df)[7] = "walk_exploration"
    colnames(df)[8] = "straightness_to_food"
    colnames(df)[9] = "straightness_back"
    colnames(df)[10] = "straightness_exploration"
    
    write.csv(df, file = paste0("/home/ceci/data/cue_learning/distance/", unique(x$unique_trial_ID),".csv")) #CHANGE DIRECTORY
...
result <- inner_join(doors, previous_door, by = "trial_n", multiple = "all") %>% 
...
track_all <- read.csv2("/home/cbaldoni/data/cue_learning/results/all_track.csv", header = TRUE) %>% 
